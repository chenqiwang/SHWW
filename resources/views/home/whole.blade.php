...
<div id="cattlist" class="am-container">
    <ul class="am-avg-sm-1 am-avg-md-3 am-avg-lg-4">
        <!--从parent.blade.php模板继承,拿来的label-->
        @foreach( $label as $q )
        @if( $q->id != 1 )
        <li>
                    ...
                    <div class="am-g">
                        ...
                        <div class="am-u-sm-4 am-u-md-5 am-u-lg-5 am-vertical-align">
                            <div class="am-vertical-align-middle">
                                <a href="{{ $q->url }}?lid={{ $q->id }}&photo={{ $q->mview }}&lname={{ $q->name }}"><img src="{{ asset('image/'.$q->mview) }}"></a>
                            </div>
                        </div>

                        <div class="am-u-sm-8 am-u-md-7 am-u-lg-7">

                            <h3><a href="{{ $q->url }}?lid={{ $q->id }}&photo={{ $q->mview }}&lname={{ $q->name }}">{{ $q->name }}</a></h3>
                            <h4>0000个关注</h4>
                            <p><span>{{ count($q) }}</span>问题</p>
                        </div>
                    </div>
                </div>
            </div>
